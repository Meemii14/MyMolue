...
from services.paystack import PaystackError, initialize_payment, verify_payment
...
def create_app():
    ...
    app.config['SQLALCHEMY_DATABASE_URI']=os.environ.get('DATABASE_URL','sqlite:///'+os.path.join(BASE_DIR,'mymolue.db'))
    app.config['SQLALCHEMY_TRACK_MODIFICATIONS']=False
    app.config['SOCKETIO_MESSAGE_QUEUE']=os.environ.get('SOCKETIO_MESSAGE_QUEUE') or None
    db.init_app(app); bcrypt.init_app(app); login_manager.init_app(app); init_realtime(app)
...
def register():
    if request.method=='POST':
        f=request.form; name=f.get('name','').strip(); email=f.get('email','').strip().lower(); phone=f.get('phone','').strip(); password=f.get('password','')
        role=f.get('role','rider') if f.get('role') in {'rider','driver'} else 'rider'
        if email==os.environ.get('ADMIN_EMAIL','').strip().lower() and os.environ.get('ADMIN_EMAIL'): role='admin'
...
def location():
    ...
    current_user.latitude=lat; current_user.longitude=lng; db.session.commit();
...
def offer(ride_id):
    ...
    db.session.commit(); socketio.emit('offer_received',{'ride_id':ride_id,'driver_id':current_user.id,'amount':amount},room=f'ride:{ride_id}'); flash('Offer sent.','success'); return redirect(url_for('dashboard'))
...
def accept(ride_id,offer_id):
    ...
    db.session.commit(); socketio.emit('ride_accepted',{'ride_id':ride_id,'driver_id':r.driver_id,'fare':r.accepted_fare},room=f'ride:{ride_id}'); return redirect(url_for('dashboard'))
...
def status(ride_id):
    ...
    db.session.commit(); socketio.emit('ride_status',{'ride_id':ride_id,'status':new},room=f'ride:{ride_id}'); return redirect(url_for('dashboard'))
...
def payment(ride_id):
    ...
    method=request.form.get('method','cash'); p=Payment.query.filter_by(ride_id=ride_id).first(); amount=r.accepted_fare or r.rider_fare
    if not p:
        p=Payment(ride_id=ride_id,amount=amount,method=method,status='paid' if method=='cash' else 'pending',reference=f'MM-{ride_id}-{int(datetime.utcnow().timestamp())}'); db.session.add(p); db.session.commit()
    if method=='paystack' and p.status!='paid':
        try:
            data=initialize_payment(current_user.email,amount,p.reference,os.environ.get('PAYSTACK_CALLBACK_URL'))
            return jsonify(payment_id=p.id,status=p.status,authorization_url=data['authorization_url'],reference=p.reference)
        except PaystackError as e: return jsonify(error=str(e)),503
    return jsonify(payment_id=p.id,status=p.status,method=p.method,amount=p.amount,reference=p.reference)
...
def verify_paystack(reference):
    ...
    try: data=verify_payment(reference)
    except PaystackError as e: return jsonify(error=str(e)),503
    if data.get('status')=='success': p.status='paid'; db.session.commit()
    return jsonify(status=p.status,reference=reference,amount=p.amount)
...
def admin():
    ...
    return jsonify(users=User.query.count(),drivers=User.query.filter_by(role='driver').count(),drivers_online=User.query.filter_by(role='driver',is_online=True).count(),rides=Ride.query.count(),completed=Ride.query.filter_by(status='completed').count(),paid=Payment.query.filter_by(status='paid').count(),revenue=sum((p.amount for p in Payment.query.filter_by(status='paid').all()),0))
